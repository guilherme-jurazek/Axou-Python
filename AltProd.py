...
<header>
    ...
    <article>
		<div class="dest">
    		<h1 class="textdest1">Alteração de produtos</h1>
		</div>

		<div class="textLabel">
		<form action="AltProd" method="GET">

			<br/>

			<label for="farq">Selecione a nova foto do seu Produto:</label>
			<input type="file" id="farq" size="40"/>

			<br/>
			<br/>

			<label for="tprod">Novo nome do Produto:</label>
			<input type="text" value="%s" id="tprod" name="nameprod" size="40" maxlength="30" placeholder="Nome do produto" autofocus="true" required="required" />

			<br/><br/>  

			<label for="preco">Novo preço do produto:</label>
			<input type="text" value="%s" id="tprod" name="precprod" size="40" maxlength="20" placeholder="R$99,99" autofocus="true" required="required"/>

			<br/><br/>


			<label for="estprod">Novo Estado do produto:</label>
'''
        ...
        html += '''
                ...
                <textarea name="desc" id="descprod" cols="60" rows="15" placeholder="Descrição..." required="required" >%s</textarea>
  ...
  <footer>
        ...
        html = html % (dados[0][1],dados[0][2],dados[0][4])
        return html

    @cherrypy.expose()
    def AltProd(self, nameprod, precprod, condicao, desc, cat, cor):
        con = sqlite3.connect('BDSite.db')
        cur = con.cursor()
        sql = "update Produtos set Nome='%s', Preco='%s', Novo='%s', Descricao='%s', CategoriaCod='%s', CodCor1='%s' where ProdCod = 1" % (str(nameprod), str(precprod), str(condicao), str(desc), str(cat), str(cor))
        ...
        if not sucesso:
            ...
            raise cherrypy.HTTPRedirect('/admin')  # refresh e atualiza a página (como se fosse um F5)
